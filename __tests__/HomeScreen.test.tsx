...
describe('HomeScreen', () => {
  ...
  it('renders correctly with all elements', () => {
    ...
    expect(getByText('Check Old TODO Lists')).toBeTruthy();
  });
  ...
  it('navigates to TodoList with isNewList=false when Check Old Lists button is pressed', () => {
    const { getByText } = render(
      <HomeScreen navigation={navigation} route={route} />,
    );

    fireEvent.press(getByText('Check Old TODO Lists'));
    expect(navigation.navigate).toHaveBeenCalledWith('TodoList', {
      isNewList: false,
    });
  });
